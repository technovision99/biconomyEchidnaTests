...
// This contract does NOT hold the assets. It's functionality is mainly that of a calculator.
// It stores info regarding each token and the limits that a LP can deposit. No checks are done
// to withdraw.
//
// The contract exposes the following functions:
//     a. beforeLiquidityAddition(), beforeLiquidityRemoval(), which are called by the LiquidityProvider 
//        contract before liquidity is added or removed.
//     b. beforeLiquidityTransfer() which is called by the LPToken before positions are transferred.
//
// The functions verify if all limits are being respected and reverts when a limit is being crossed.
